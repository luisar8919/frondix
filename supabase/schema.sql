...
create policy "ver mi empresa" on empresas for select
  using (id in (select empresa_id from miembros where user_id = auth.uid()));

create policy "ver miembros de mi empresa" on miembros for select
  using (empresa_id in (select empresa_id from miembros where user_id = auth.uid()));

create policy "dueno/admin gestiona miembros" on miembros for all
  using (empresa_id in (select empresa_id from miembros where user_id = auth.uid() and rol in ('dueno', 'admin')));
...
create policy "crear datasets en mi empresa" on datasets for insert
  with check (empresa_id in (select empresa_id from miembros where user_id = auth.uid()));

create policy "ver records de mi empresa" on records for select
  using (dataset_id in (select id from datasets where empresa_id in (select empresa_id from miembros where user_id = auth.uid())));

create policy "crear records en mi empresa" on records for insert
  with check (dataset_id in (select id from datasets where empresa_id in (select empresa_id from miembros where user_id = auth.uid())));
